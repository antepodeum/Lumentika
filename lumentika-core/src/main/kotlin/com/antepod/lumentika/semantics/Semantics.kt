...
public enum class SemanticAction { CLICK, LONG_CLICK, SET_VALUE, INCREMENT, DECREMENT, FOCUS, CLEAR_FOCUS, SCROLL_FORWARD, SCROLL_BACKWARD, SET_SELECTION, COPY, CUT, PASTE }
public enum class LiveRegion { NONE, POLITE, ASSERTIVE }
public data class SemanticRange(val current: Float, val minimum: Float, val maximum: Float, val step: Float? = null)
public data class CollectionInfo(val rows: Int, val columns: Int, val hierarchical: Boolean = false)
public data class CollectionItemInfo(val row: Int, val column: Int, val rowSpan: Int = 1, val columnSpan: Int = 1)
public data class SemanticsConfiguration(
    val role: SemanticRole = SemanticRole.NONE, val label: String? = null, val value: String? = null,
    val stateDescription: String? = null, val hint: String? = null, val enabled: Boolean = true,
    val selected: Boolean? = null, val checked: Boolean? = null, val expanded: Boolean? = null,
...
public class SemanticsRuntime(private val root: Element) {
    ...
    public var accessibilityFocus: SemanticsNodeId? = null; private set
    public var artifact = SemanticsArtifact(0, emptyList(), emptyMap(), null); private set
    public fun configure(element: Element, config: SemanticsConfiguration) { configs[element] = config; ids.getOrPut(element) { SemanticsNodeId(nextId.incrementAndGet()) } }
    public fun commit(hit: HitTestArtifact): SemanticsChangeSet {
        ...
        val roots = walk(root); generation++
        artifact = SemanticsArtifact(generation, roots, nodes, accessibilityFocus?.takeIf(nodes::containsKey))
        val changes = SemanticsChangeSet(nodes.keys - previous.keys, previous.keys - nodes.keys, nodes.keys.filterTo(linkedSetOf()) { previous[it] != nodes[it] })
        accessibilityFocus = artifact.accessibilityFocus
        return changes
    }
    public fun perform(id: SemanticsNodeId, action: SemanticAction, argument: Any? = null): Boolean = artifact.nodes[id]?.config?.actions?.get(action)?.invoke(argument) ?: false
    public fun requestAccessibilityFocus(id: SemanticsNodeId): Boolean { if (id !in artifact.nodes) return false; accessibilityFocus = id; return true }
    public companion object { private val nextId = AtomicLong() }
}
