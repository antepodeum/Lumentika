...
public enum class EventType {
    POINTER_DOWN, POINTER_MOVE, POINTER_UP, POINTER_ENTER, POINTER_LEAVE, POINTER_CANCEL, WHEEL,
    KEY_DOWN, KEY_UP, FOCUS, BLUR, FOCUS_IN, FOCUS_OUT,
}
...
public open class BaseEvent(
    ...
    override fun stopPropagation() { propagationStopped = true }
    override fun stopImmediatePropagation() { propagationStopped = true; immediatePropagationStopped = true }
    override fun preventDefault() { if (cancelable) defaultPrevented = true }
}

public enum class PointerType { MOUSE, TOUCH, PEN, UNKNOWN }
...
public enum class LogicalKey { TAB, ENTER, SPACE, ESCAPE, ARROW_LEFT, ARROW_RIGHT, ARROW_UP, ARROW_DOWN, HOME, END, BACKSPACE, DELETE, CHARACTER, UNKNOWN }
...
public class EventDispatcher(private val root: Element) {
    ...
    public fun dispatch(type: EventType, event: BaseEvent): Boolean {
        ...
        require(target.isMounted && isInRoot(target)) { "Cannot dispatch to stale/out-of-root element" }
        val actualEvent = if (target === event.target) event else retarget(event as PointerEvent, target)
        ...
        if (!actualEvent.propagationStopped) {
            ...
            if (!actualEvent.immediatePropagationStopped) invoke(target, type, actualEvent, EventPhase.TARGET, capture = false)
        }
        if (!actualEvent.propagationStopped) {
        ...
        if (type == EventType.POINTER_UP || type == EventType.POINTER_CANCEL) pointerCapture.remove((actualEvent as? PointerEvent)?.pointerId)
        return !actualEvent.defaultPrevented
    }

    public fun repairRemovedSubtree(subtree: Element) {
        pointerCapture.entries.removeIf { (_, element) -> element === subtree || isDescendant(element, subtree) }
    ...
    private fun invoke(element: Element, type: EventType, event: BaseEvent, phase: EventPhase, capture: Boolean) {
        event.currentTarget = element
        event.phase = phase
        event.immediatePropagationStopped = false
        listeners[element]?.get(type)?.toList()?.filter { it.capture == capture }?.forEach {
            it.callback(event)
    ...
    private fun path(target: Element): List<Element> {
        ...
        while (current != null) { result.addFirst(current); current = current.parent }
        return result.toList()
    }

    private fun isInRoot(element: Element): Boolean = element === root || isDescendant(element, root)
    private fun isDescendant(element: Element, ancestor: Element): Boolean {
        var current = element.parent
        while (current != null) { if (current === ancestor) return true; current = current.parent }
        return false
    }

    private fun retarget(event: PointerEvent, target: Element): PointerEvent = PointerEvent(
        target, event.pointerId, event.pointerType, event.position, event.button, event.buttons,
        event.pressure, event.timestampNanos, event.modifiers, event.historical, event.cancelable,
    )
}

public enum class FocusCause { POINTER, KEYBOARD, PROGRAMMATIC, REPAIR }
public data class FocusProperties(val focusable: Boolean = false, val disabled: Boolean = false, val tabIndex: Int = 0)
...
public class FocusManager(
    ...
    public fun focus(element: Element, cause: FocusCause = FocusCause.PROGRAMMATIC) {
        ...
        require(element.isMounted && config.focusable && !config.disabled) { "Element is not focusable" }
    ...
    private fun ordered(): List<Element> {
        ...
        return logical.filter { properties[it]?.let { value -> value.focusable && !value.disabled && value.tabIndex >= 0 } == true }
            .sortedWith(compareBy<Element> { properties[it]?.tabIndex?.takeIf { index -> index > 0 } ?: Int.MAX_VALUE }.thenBy { logical.indexOf(it) })
    }

    private fun walk(element: Element, result: MutableList<Element>) { result += element; element.children.forEach { walk(it, result) } }
    private fun isDescendant(element: Element, ancestor: Element): Boolean {
        var current = element.parent
        while (current != null) { if (current === ancestor) return true; current = current.parent }
        return false
