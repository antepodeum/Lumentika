...
public sealed interface ImageSource {
    public data class Bytes(val bytes: ByteArray, val mimeType: String) : ImageSource {
        override fun equals(other: Any?): Boolean = other is Bytes && mimeType == other.mimeType && bytes.contentEquals(other.bytes)
...
public data class TextContent(var text: String) : Content, IntrinsicMeasurable {
    override fun measure(input: IntrinsicMeasureInput): Size {
        ...
        val width = input.knownWidth ?: input.availableWidth?.let { minOf(it, naturalWidth) } ?: naturalWidth
...
public data class ImageContent(val source: ImageSource, val intrinsicSize: Size? = null) : Content, IntrinsicMeasurable {
    ...
    override fun record(recorder: PaintRecorder, bounds: Rect) = recorder.record(PaintCommand.DrawImage(source, bounds))
}
...
public open class Element(public val kind: String = "element") : AutoCloseable {
    ...
    public val children: List<Element> get() = mutableChildren
    ...
    public fun <T : Any> attach(key: AttachmentKey<T>, value: T) { attachments[key] = value }
...
public open class UiScope(public val parent: Element) {
    public fun element(kind: String = "element", content: Content? = null, block: UiScope.() -> Unit = {}): Element {
